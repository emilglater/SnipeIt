...
/**
 * @brief Allocate and initialise a pose ring buffer.
 *
 * @param capacity Number of frame->pose entries to retain. Pass
 *                 POSE_RING_DEFAULT_CAPACITY unless you have a reason not to.
 *
 * @details The caller must eventually call pose_ring_destroy().
 *
...
/**
 * @brief Free all memory associated with a pose ring.
 *
 * @param r Ring to destroy. May be NULL (no-op).
 */
void pose_ring_destroy(PoseRing *r);

/**
 * @brief Record the capture-time pose for a frame_id.
 *
 * @param r             The ring.
 * @param frame_id      The id tagged onto the outgoing frame.
 * @param hor_angle     Servo pan angle at capture, degrees.
 * @param ver_angle     Servo tilt angle at capture, degrees.
 * @param capture_ts_ms CLOCK_MONOTONIC milliseconds at capture.
 *
 * @details Called by the sender once per outgoing frame, at the instant of
 *          capture. Overwrites the oldest entry when the ring is full.
...
/**
 * @brief Join a returned frame_id back to its capture-time pose.
 *
 * @param r        The ring.
 * @param frame_id The frame_id echoed back in the detection message.
 * @param out      Filled with the matching entry on success. Untouched on a
 *                 miss.
 *
 * @details Called by the detection receiver when a message arrives from the
 *          Orin. A false return is normal and means "no pose available -> do
 *          not aim on this one". Thread-safe: acquires the internal mutex.
 *
 * @returns true if the frame_id was still in the buffer; false if it has been
 *          overwritten (detection arrived too late) or was never recorded.
 */
bool pose_ring_lookup(PoseRing *r, uint32_t frame_id, PoseEntry *out);

/**
 * @brief Total entries recorded since creation (monotonic).
 *
 * @param r The ring.
 *
 * @details Useful for diagnostics and tests. Thread-safe: reads under mutex.
 *
 * @returns The running total of recorded entries.
 */
