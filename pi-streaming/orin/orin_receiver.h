...
/**
 * @brief Handler invoked once per received detection message.
 *
 * @param msg  The parsed detection message (valid only for the call's
 *             duration).
 * @param pose The capture-time pose joined from the pose ring by frame_id, or
 *             NULL if the message carried no frame_id or the frame had already
 *             aged out of the ring (detection arrived too late to aim on).
 * @param user The opaque pointer passed to orin_receiver_start().
 */
...
/**
 * @brief Bind the PULL socket and start the receive thread.
 *
 * @param endpoint ZeroMQ bind endpoint, e.g. "tcp://0.0.0.0:5556".
 * @param ring     Pose ring to join frame_id -> pose against. Borrowed, not
 *                 owned; must outlive the receiver.
 * @param handler  Called for each parsed message (see the threading contract
 *                 at the top of this header).
 * @param user     Opaque pointer forwarded to the handler.
 *
 * @returns An opaque handle on success, or NULL on failure (bad args, zmq
 *          setup failure, bind failure, or thread spawn failure — fully
 *          cleaned up).
 */
...
/**
 * @brief Stop the receive thread and free all resources.
 *
 * @param r Receiver handle. May be NULL (no-op).
 */
void orin_receiver_stop(OrinReceiver *r);

/**
 * @brief Read the diagnostic counters (monotonic since start).
 *
 * @param r              Receiver handle. NULL is a no-op.
 * @param received       Messages received. May be NULL.
 * @param parse_failures Messages that failed to parse. May be NULL.
 * @param pose_misses    Messages with no matching pose. May be NULL.
 *
 * @details Read racily off the receive thread: fine for reporting, not for
 *          control.
 */
